    ...
    def generate_report(self) -> ResearchReport:
        ...
        for finding in self.findings:
            if finding.source_url and finding.source_url not in seen_urls:
                ...
                sources.append(
                    {"url": finding.source_url, "title": finding.source_title or "Untitled"}
                )
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if not self.findings:
            recommendations.append(
                "Consider refining the research topic or using alternative search terms."
            )
...
async def deep_research(
    topic: str, goals: list[str] | None = None, max_steps: int = 10, max_depth: int = 3
) -> str:
    ...
    with log_tool_call("deep_research", topic) as tool_log:
        logger.info(
            "Tool deep_research: %s (goals: %s, max_steps: %d, max_depth: %d)",
            topic,
            goals,
            max_steps,
            max_depth,
        ...
        try:
                ...
                if step.next_queries and agent.max_depth > 0:
                    for follow_up in step.next_queries[:1]:  # Limit follow-ups
                        ...
                        follow_step = ResearchStep(
                            step_number=executed_steps + 1, description=follow_up, query=follow_up
                        )
            ...
            result = "\n".join(output_parts)
            tool_log.log_result(
                f"{len(report.findings)} findings, {report.confidence:.0%} confidence"
            )
            return result
...
async def quick_research(topic: str) -> str:
    ...
    with log_tool_call("quick_research", topic) as tool_log:
        ...
        try:
            ...
            output_parts = [f"# Quick Research: {topic}", "", f"**Findings**: {len(findings)}", ""]
...
def _parse_search_results(results: str) -> list[ResearchFinding]:
    ...
    while i < len(lines):
        ...
        if line.startswith("- "):
            ...
            current_finding = ResearchFinding(title=line[2:].strip(), content="")
