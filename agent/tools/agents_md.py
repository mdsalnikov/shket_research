...
def _parse_markdown_headings(content: str) -> list[AgentsMdSection]:
    ...
    for line in lines:
        ...
        if match:
            ...
            current_section = AgentsMdSection(title=title, content="", level=level)
        else:
...
def _extract_env_vars(content: str) -> list[dict[str, str]]:
    ...
    for match in re.finditer(table_pattern, content, re.MULTILINE):
        ...
        if var_name and description:
            env_vars.append({"name": var_name, "description": description, "required": "No"})
...
def _extract_commands(content: str) -> list[dict[str, str]]:
    ...
    for match in re.finditer(bash_pattern, content, re.MULTILINE | re.DOTALL):
        ...
        if first_line and not first_line.startswith("#"):
            commands.append({"command": first_line, "context": code})
        ...
        try:
            ...
            if doc.env_vars:
                ...
                for env in doc.env_vars:
                    req = (
                        " (required)" if env.get("required", "").lower() in ("yes", "true") else ""
                    )
                    output_parts.append(
                        f"- `{env.get('name', '')}`{req}: {env.get('description', '')}"
                    )
                output_parts.append("")
