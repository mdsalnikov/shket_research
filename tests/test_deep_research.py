...
def test_research_plan():
    ...
    plan = ResearchPlan(topic="test topic", goals=["goal 1", "goal 2"], estimated_steps=5)
...
async def test_deep_research_basic():
    ...
    result = await deep_research(
        "python programming", goals=["find basics"], max_steps=3, max_depth=1
    )
...
async def test_deep_research_limited_steps():
    ...
    result = await deep_research("test topic", max_steps=2, max_depth=1)
