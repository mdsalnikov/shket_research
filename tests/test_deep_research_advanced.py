...
async def test_verify_finding_with_mock():
    ...
    with patch('agent.tools.deep_research.web_search', new_callable=AsyncMock) as mock_search:
        mock_search.return_value = "- Python Statistics\n  https://example.com/popularity\n  Python is widely used"
...
def test_synthesize_findings_single_theme():
    ...
    agent.findings = [
        ...
        ResearchFinding(title="Machine Learning Applications", content="ML is used in many domains"),
        ResearchFinding(title="Machine Learning Tools", content="Popular ML tools include TensorFlow"),
    ]
